...
// // the menu button svg is displayed on incorrect comment. it only displays correctly after refresh
...
// const CommentMenu: React.FC<CommentMenuProps> = ({ id, userId }) => {
//     useAuth();
//     const authState = useRecoilValue(isLoggedIn);
//     const setSelectedCommentId = useSetRecoilState(selectedCommentIdState);
//     const [comments, setComments] = useRecoilState(currCommentState);
//     const setIsEditComment = useSetRecoilState(editCommentState);
//     const authUserIdLoadable = useRecoilValueLoadable(currUserIdSelector);
//     const setAuthUserId = useSetRecoilState(currUserId);
//     const [cookie, setCookie] = useCookies(['jwt']);

//     useEffect(() => {
//         if (authUserIdLoadable.state === 'hasValue') {
//             setAuthUserId(authUserIdLoadable.contents);
//         }
//     }, [authUserIdLoadable.state]);
...
//     const handleDeleteComment = async () => {
//         try {
//             await axios.delete(`http://localhost:3000/api/comment/deletecomment/?userId=${userId}&id=${id}`);
//             const updatedComments = comments.filter(comment => comment.id !== id);
//             setComments(updatedComments);
//             setSelectedCommentId(null);
//         } catch (error) {
...
//     console.log("COMMENTMENU userid: ", authUserIdLoadable.contents?.userID)

//     return (
//         <div>
//             {/* authState.isAuthenticated && userId === authUserId.userID */}
//             {authState.isAuthenticated && userId === authUserIdLoadable.contents?.userID && (
//                 <div className="flex items-center w-64">
//                     <DropdownMenu>
//                         <DropdownMenuTrigger>
//                             <svg
